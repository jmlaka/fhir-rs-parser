...
impl Measure<'_> {
    ...
    pub fn supplemental_data(&self) -> Option<Vec<Measure_SupplementalData>> {
        if let Some(Value::Array(val)) = self.value.get("supplementalData") {
            return Some(
                val.into_iter()
                    .map(|e| Measure_SupplementalData {
                        value: Cow::Borrowed(e),
                    })
                    .collect::<Vec<_>>(),
            );
...
impl MeasureStatus {
    ...
    pub fn to_string(&self) -> String {
        match self {
            MeasureStatus::Draft => "draft".to_string(),
            MeasureStatus::Active => "active".to_string(),
            MeasureStatus::Retired => "retired".to_string(),
            MeasureStatus::Unknown => "unknown".to_string(),
        }
    }
}
